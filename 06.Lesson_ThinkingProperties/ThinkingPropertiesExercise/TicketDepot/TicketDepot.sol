...
contract TicketDepot {
    ...
    {
        if (msg.sender == eventsMap[_eventID].owner || msg.value < eventsMap[_eventID].ticketPrice + transactionFee) {
            revert();
        }
    ...
    function offerTicket(uint16 _eventID, uint16 _ticketID, uint64 _price, address _buyer, uint16 _offerWindow)
        ...
        payable
    {
        if (msg.sender != eventsMap[_eventID].attendees[_ticketID] || msg.value < transactionFee) revert();
    ...
    function buyOfferedTicket(uint16 _eventID, uint16 _ticketID, address _newAttendee) public payable {
        bytes32 offerID = keccak256(abi.encode(_eventID, _ticketID));
        if (
            msg.value < offerings[offerID].price || block.number >= offerings[offerID].deadline
                || (offerings[offerID].buyer != address(0) && offerings[offerID].buyer != msg.sender)
        ) revert();
