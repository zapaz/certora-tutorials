...
contract AuctionImpl is TokenInterface {
    ...
    */
    ...
    struct AuctionStrcut {
        uint256 prize; // the prize decreasing by every bid
        uint256 payment; // the payment to be paid by the last winner
        address winner; //the current winner
        uint256 bid_expiry;
        uint256 end_time;
    }

    mapping(uint256 => AuctionStrcut) auctions;

    function getAuction(uint256 id) public view returns (uint256, uint256, address, uint256, uint256) {
        return (
            auctions[id].prize,
            auctions[id].payment,
            auctions[id].winner,
            auctions[id].bid_expiry,
            auctions[id].end_time
        );
    }

    function newAuction(uint256 id, uint256 payment) public authorized {
    ...
    function bid(uint256 id, uint256 b) public {
