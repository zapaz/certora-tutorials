...
contract ReserveList is IReserveList {
    ...
    function removeReserve(address token) external override {
        require(token != address(0), "invalid token 0x0");

        uint256 index = reserves[token].id;
        require(underlyingList[index] == token, "token is not in the system");

        reserveCount = reserveCount - 1;

        if (index != reserveCount) {
            address lastToken = underlyingList[reserveCount];
            underlyingList[index] = lastToken;
            reserves[lastToken].id = index;
        }

        underlyingList[reserveCount] = address(0);
