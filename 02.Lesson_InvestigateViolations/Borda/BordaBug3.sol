...
contract Borda is IBorda {
    ...
    function getFullVoterDetails(address voter)
        external
        view
        override
        returns (uint8 age, bool registered, bool voted, uint256 vote_attempts, bool black_listed)
    {
        Voters memory voter_details = _voters[voter];
        return (
            voter_details.age,
            voter_details.registered,
            voter_details.voted,
            voter_details.vote_attempts,
            voter_details.black_listed
        );
    }

    function getFullContenderDetails(address contender)
        external
        view
        override
        returns (uint8 age, bool registered, uint256 points)
    {
    ...
    function vote(address first, address second, address third) public override returns (bool) {
        ...
        require(
            _contenders[first].registered && _contenders[second].registered && _contenders[third].registered,
            "one or more of the specified addresses aren't registered as contenders"
        );
        require(
            first != second && first != third && second != third, "you've tried to vote for the same more than once"
        );
        ...
        if (voter_details.voted) {
            require(
                voter_details.vote_attempts >= 3,
                "you've already voted. If you reach 3 attempts you will be black listed"
            );
